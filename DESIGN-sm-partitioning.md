...
> **Status: archived as a future extension** (April 2026). This was the project's original direction. The current arc has pivoted to post-hoc MHA→MLA conversion (see [`DESIGN.md`](./DESIGN.md)) because the MLA route is more legible as a single-shot deliverable, doesn't gamble on Green Context support being available on consumer Blackwell, and reuses the same engine substrate. The SM-partitioning research question below is still open and remains a strong candidate for a Stage 2 follow-on or a Stage C pivot if the MLA-conversion outcome warrants it.
>
> The Stage A engine described here matches what was actually built in the current arc, so this design doc is still load-bearing for any future revival of this direction.
